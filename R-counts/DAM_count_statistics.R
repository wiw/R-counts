...
# Declare variables
###################
prefixDir <- "ludo" # directory for other experiments
...
ScatterPlottingOnAveraged <- function(dataSet, tag) {
  for (j in 8:(ncol(dataSet))){
  	for (i in 8:(ncol(dataSet))){
  		if (j != i) {
			  bmp(filename=file.path(prefixDir, outputScttr, paste("scatter_", j, "_vs_", i, "_", name, "_", tag, ".bmp", sep="")), width=600, height=600, units = "px")
...
##################################
ScatterPlottingOnAveraged(dataSet=DATAs.norm[[name]], tag=name)
...
################################
ScatterPlottingOnAveraged(dataSet=DATAs.norm.ave[[name]], tag="data_ave")
}
print("I found that I calculate only one protein data, because I don't run Correlation counts and Scatter plots")
